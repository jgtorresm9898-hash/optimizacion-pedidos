...
Cada vehiculo fisico puede hacer maximo 1 viaje por dia.
"""
...
def parse_orders(pedidos_file):
    ...
    for col_idx, val in enumerate(day_row):
        if pd.notna(val):
            ...
            for d in DAY_ORDER:
                if val_upper == d or val_upper.replace('E', 'E') == d:
    ...
    for (zone, port), farm_data in sorted_routes:
        ...
        def tag(trips_list, dest, ttype):
            for t in trips_list:
                ...
                t['destination']        = dest
                t['trip_type']          = ttype
                t['farm_pallets_size']  = farm_pallets_size
        ...
        consol_candidates = [f for f in farm_data if f in CONSOLIDACION_ROUTES]

"""
Página: Optimización semanal.

Sube el pedido de la semana completa y propone la ruta más barata para
despachar todas las fincas, incluyendo sugerencias de mover pallets entre
días (consolidación) cuando eso reduce el costo total de transporte.
"""
import re
import io
import copy
...
from opt_engine import (
    ...
    suggest_consolidations, apply_consolidation,
...
def _run_optimization(semana_num, unavailable_by_day):
    excel_bytes, summary = generate_excel_bytes(
        st.session_state.working_orders, semana_num,
    ...
    st.session_state.suggestions = suggest_consolidations(
        st.session_state.working_orders,
        unavailable_vehicle_ids_by_day=unavailable_by_day,
    )
    st.session_state.unavailable_by_day = unavailable_by_day
    return True
...
def render():
    ...
    # Copia de trabajo del pedido: persiste entre reruns para poder aplicar
    # ajustes de consolidación (sugerencias aceptadas) sin perder el progreso.
    file_key = (uploaded.name, len(file_bytes))
    if st.session_state.get('orders_source') != file_key:
        st.session_state.orders_source  = file_key
        st.session_state.working_orders = copy.deepcopy(orders)
        st.session_state.pop('last_result', None)
        st.session_state.pop('suggestions', None)

    working_orders = st.session_state.working_orders

    days = sorted(working_orders.keys(), key=lambda d: DAY_ORDER.index(d) if d in DAY_ORDER else 99)
    ...
    if run or 'last_result' in st.session_state:
        if run:
            ...
            with st.spinner("Calculando rutas óptimas..."):
                ok = _run_optimization(semana_num, unavailable_by_day)
        ...
        # ── Sugerencias de consolidación entre días ──────────────
        suggestions = st.session_state.get('suggestions', [])
        if suggestions:
            st.markdown("#### 💡 Sugerencias de consolidación")
            st.caption(
                "Mover parte del pedido de una finca a un día cercano (misma finca y "
                "mismo puerto) puede ahorrar costos de transporte, sin perder cajas."
            )
            for idx, sug in enumerate(suggestions):
                with st.container(border=True):
                    c1, c2 = st.columns([5, 1])
                    tipo      = "el pedido completo" if sug['is_full'] else "parte del pedido"
                    money_str = f"${sug['savings']:,.0f}".replace(",", ".")
                    cajas_str = f"{sug['cajas']:,}".replace(",", ".")
                    c1.markdown(
                        f"Junta **{tipo}** de **{sug['farm']}** ({sug['port']}) del "
                        f"**{sug['from_day'].capitalize()}** al **{sug['to_day'].capitalize()}**: "
                        f"{sug['pallets']}P / {cajas_str} cajas — "
                        f"Ahorro estimado: **{money_str}**"
                    )
                    if c2.button("✅ Aceptar", key=f"accept_sug_{idx}"):
                        apply_consolidation(st.session_state.working_orders, sug)
                        with st.spinner("Recalculando rutas..."):
                            _run_optimization(semana_num, st.session_state.unavailable_by_day)
                        st.rerun()
